...
const SubmitButton: React.FC<SubmitButtonProps> = ({
  ...
  return (
    <button
      type="submit"
      disabled={loading}
      className={`text-white px-4 h-full py-2 w-full rounded-lg border cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${variantClasses[variant]} ${className}`}
    >
      {loading ? (
        <LoadingSpinner
          text={loadingLabel}
